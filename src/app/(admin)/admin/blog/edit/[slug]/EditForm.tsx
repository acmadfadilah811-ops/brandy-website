...
export default function EditForm({ post }: EditFormProps) {
  ...
  // Helper to extract body content to plain text for simple textarea editing
  ...
  const [bodyText, setBodyText] = useState(getBodyTextContent());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || "Gagal memperbarui artikel.");
      }
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-slate-500">
          Ubah judul, konten, atau pengaturan optimasi mesin pencari untuk artikel Anda.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-5">
            ...
            {/* Slug (Disabled for Edit to maintain URL stability in SEO) */}
            <div>
              <label htmlFor="slug" className="block text-xs font-600 uppercase tracking-wider text-slate-500 mb-2">
                URL Slug (Tidak dapat diubah demi stabilitas indeks SEO)
              </label>
              <input
                id="slug"
                type="text"
